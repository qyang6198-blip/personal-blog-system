...
(function(){
  ...
  s.textContent = [
    ...
    "[data-theme=dark] .page-dim-overlay{--dim-bg:rgba(0,0,0,0.3)}",
    "body.page-dimmed main{filter:blur(8px);transition:filter 0.5s cubic-bezier(0.16,1,0.3,1)}",
    ".he-sbtn{position:relative;display:flex;align-items:center;gap:0}",
    ".he-sbtn input{width:0;padding:0;border:0;outline:none;font-size:14px;font-family:inherit;background:transparent;border-radius:8px;transition:width 0.35s cubic-bezier(0.16,1,0.3,1),padding 0.35s cubic-bezier(0.16,1,0.3,1);opacity:0}",
    ".he-sbtn.on input{width:200px;padding:6px 12px;opacity:1;background:var(--tag-bg,rgba(0,0,0,0.04));border:1px solid var(--border-light,rgba(0,0,0,0.06))}",
    "[data-theme=dark] .he-sbtn.on input{background:rgba(255,255,255,0.04);border-color:rgba(255,255,255,0.08)}",
    ".he-sbtn .he-sb-icon{cursor:pointer;display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;color:var(--text-secondary);transition:color 0.2s}",
    ".he-sbtn .he-sb-icon:hover{color:var(--accent);background:rgba(184,148,74,0.08)}",
    ".header-srch-open .header-nav a{transform:translateX(-100px);transition:transform 0.35s cubic-bezier(0.16,1,0.3,1)}",
  ].join("\n");
  document.head.appendChild(s);
  ...
  // Fresh search feature
  (function(){
    var _prevBtn = header.querySelector(".search-btn, .header-icon-btn[aria-label*=\"\u641c\"]");
    if(_prevBtn) _prevBtn.style.display = "none";

    var sb = document.createElement("div");
    sb.className = "he-sbtn";
    var ic = document.createElement("span");
    ic.className = "he-sb-icon";
    ic.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>';
    var ip = document.createElement("input");
    ip.type = "text"; ip.placeholder = "\u641c\u7d22\u6587\u7ae0..."; ip.autocomplete = "off";
    sb.appendChild(ic);
    sb.appendChild(ip);

    var _thm = document.getElementById("theme-toggle");
    var navEl = header.querySelector(".header-nav");
    if(navEl && _thm) navEl.insertBefore(sb, _thm);
    else if(navEl) navEl.appendChild(sb);

    ic.addEventListener("click", function(e){
      e.stopPropagation();
      var on = sb.classList.toggle("on");
      if(on){ ip.focus(); header.classList.add("header-srch-open"); }
      else { ip.blur(); ip.value=""; header.classList.remove("header-srch-open"); }
    });
    ...
    ip.addEventListener("keydown", function(e){
      ...
      if(e.key === "Escape"){
        sb.classList.remove("on"); ip.value=""; header.classList.remove("header-srch-open");
      }
    });

    document.addEventListener("click", function(e){
      if(sb.classList.contains("on") && !sb.contains(e.target)){
        sb.classList.remove("on"); ip.value=""; header.classList.remove("header-srch-open");
      }
    });
  })();
